...
# Background Tasks for fetching data from yfinance in background
# Depends for dependency injection for database connectivity.
from fastapi.templating import Jinja2Templates
from database import SessionLocal, engine
from sqlalchemy.orm import Session
from pydantic import BaseModel
# pydantic to automatically verify the datatypes in requests.
from models import Stock
import yfinance
# yfinance is library for yahoo finance to get stock data.
...
async def create_stock(stock_request: StockRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    return {
        "code": "success",
        "message": "stock added to the database"
    }